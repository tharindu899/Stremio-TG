...
from Backend.helper.caption_manager import apply_filename_caption
...
from Backend.helper.metadata import extract_default_id, metadata_from_caption_or_filename
...
def _is_supported_media(message: Message) -> bool:
    ...
    if message.document:
        ...
        if detect_split_upload(file_name, mime_type):
            return True
        if find_split_source(file_name, message.caption or "")[1]:
            return True
    return False
...
async def file_receive_handler(client: Client, message: Message):
    ...
    if is_manual and session and session.get("kind") == "personal":
        ...
        if SettingsManager.current().auto_filename_caption:
            await apply_filename_caption(message)
        return
    ...
    try:
        ...
        if not _is_supported_media(message):
            if SettingsManager.current().auto_filename_caption:
                await apply_filename_caption(message)
            LOGGER.debug(f"Ignoring unsupported channel file {message.id}")
            return

        _, title, msg_id, raw_size, size, channel = _extract_fields(message)

        raw_filename = getattr(message.video or message.document, "file_name", "") or title
        split_override = None
        if message.document:
        ...
        metadata_info = await metadata_from_caption_or_filename(
            ...
            channel=int(channel),
            msg_id=msg_id,
            override_id=override_id or extract_default_id(message.caption or ""),
        ...
        title = metadata_info.get("media_filename") or _finalize_title(title, metadata_info)

        await file_queue.put((metadata_info, int(channel), msg_id, size, raw_size, title))

        if SettingsManager.current().auto_filename_caption:
            await apply_filename_caption(message)
        elif is_real_session:
            create_task(stamp_caption_with_id(message, metadata_info))
    except FloodWait as e:
        # Do not reply to the channel during a FloodWait; that send can trigger
        # another FloodWait and leave an unhandled dispatcher exception.
        LOGGER.warning(f"Telegram FloodWait in file handler: sleeping {e.value}s")
        await asleep(e.value + 1)
    except Exception as e:
        LOGGER.error(f"Error handling channel file {getattr(message, 'id', '?')}: {e}")
