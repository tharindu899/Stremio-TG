...
async def _identify(name: str):
    ...
    is_tv = bool(season is not None and episode is not None and not isinstance(season, list) and not isinstance(episode, list))
    if is_tv:
    ...
    try:
        ...
        await db.dbs["tracking"]["subtitles"].update_one(
            ...
            {"$set": {
                ...
                "season": int(season) if season is not None and str(season) != "" else None,
                "episode": int(episode) if episode is not None and str(episode) != "" else None,
                "lang_code": code,
                "lang_label": label,
                "name": name,
                "chat_id": int(channel),
...
async def get_subtitles_for(imdb_id: str, media_type: str, season, episode):
    ...
    if media_type == "tv":
        query["season"] = int(season) if season is not None and str(season) != "" else None
        query["episode"] = int(episode) if episode is not None and str(episode) != "" else None
