...
# ═════════════════════════════════════════════════════════════════════════════
#  DuplicateManager — find repeated stream rows and remove selected old files
# ═════════════════════════════════════════════════════════════════════════════
class DuplicateManager:
    ...
    def _dup_key(quality: dict) -> tuple:
        """Match the upstream duplicate identity without requiring DB internals."""
        name = re.sub(r"\s+", " ", str(quality.get("media_filename") or quality.get("name") or "").strip().lower())
        size = str(quality.get("size") or "").strip().lower()
        part_count = len(quality.get("parts") or [])
        return (str(quality.get("quality") or "").strip().lower(), name, size, part_count)

    def get_status(self) -> Dict[str, Any]:
        s = self.state
        elapsed = 0.0
        if s["started_at"]:
            elapsed = max(0.0, (s["finished_at"] or _now()) - s["started_at"])
        ...
        if s.get("purge_started_at"):
            p_elapsed = max(0.0, (s.get("purge_finished_at") or _now()) - s["purge_started_at"])
        p_eta = 0
        ...
        return {
            ...
            "purge_total": p_total,
            "purge_done": p_done,
            "purge_progress": round(p_done / p_total * 100) if p_total else 0,
    ...
    async def start(self) -> Dict[str, Any]:
        if self._db is None:
            return {"ok": False, "message": "Database is not ready yet."}
        async with self._lock:
            if self.state["status"] == "running":
        ...
        for items in buckets.values():
            ...
            self.state["groups"].append({
                ...
                "entries": [
                    {
                        "id": it["id"],
                        "name": it.get("media_filename") or it.get("name"),
                        "size": it.get("size"),
                        "parts": len(it.get("parts") or []),
                    }
                    for it in items
                ],
            })
    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            for i in range(1, db.current_db_index + 1):
                ...
                if storage is None:
                    continue
                async for movie in storage["movie"].find({}, {"title": 1, "release_year": 1, "telegram": 1}):
                    if self._cancel:
                        break
                    s["scanned"] += 1
                ...
                if self._cancel:
                    break
                async for show in storage["tv"].find({}, {"title": 1, "seasons": 1}):
                    if self._cancel:
                        break
                    s["scanned"] += 1
                    ...
                    for season in show.get("seasons", []):
                        for ep in season.get("episodes", []):
                            label = f"{title} S{int(season.get('season_number', 0)):02d}E{int(ep.get('episode_number', 0)):02d}"
                            gid = self._collect(ep.get("telegram", []), label, "tv", gid)
            s["status"] = "cancelled" if self._cancel else "completed"
            s["finished_at"] = _now()
            LOGGER.info("[Duplicates] %s — %s group(s), %s redundant", s["status"], len(s["groups"]), s["duplicate_count"])
        except asyncio.CancelledError:
            s["status"] = "cancelled"
            s["finished_at"] = _now()
            raise
            ...
            s["error"] = str(exc)
            s["finished_at"] = _now()
            LOGGER.error("[Duplicates] Error: %s", exc)
        ...
        async with self._lock:
            ...
            if delete_all:
                for group in self.state.get("groups", []):
                    # Keep the newest occurrence (last in stored stream order).
                    ids.extend(entry["id"] for entry in group.get("entries", [])[:-1])
            elif stream_ids:
                ids = list(stream_ids)
            ...
            self.state.update({
                ...
                "purge_total": len(ids),
                "purge_done": 0,
                "purge_started_at": _now(),
                "purge_finished_at": 0.0,
            })
            self._purge_task = asyncio.create_task(self._run_purge(ids))
            return {"ok": True, "message": f"Removing {len(ids)} duplicate(s)…", "total": len(ids), "status": self.get_status()}
    ...
    async def _run_purge(self, ids: List[str]) -> None:
        ...
        try:
            for stream_id in ids:
                try:
                    if await self._db.delete_media_by_stream_id(stream_id, delete_file=True):
                        purged += 1
                except Exception as exc:
                    LOGGER.error("[Duplicates] cleanup failed for %s: %s", stream_id, exc)
                s["purge_done"] += 1
            removed = set(ids)
            groups = []
            for group in s.get("groups", []):
                remaining = [entry for entry in group.get("entries", []) if entry.get("id") not in removed]
                if len(remaining) >= 2:
                    group["entries"] = remaining
                    groups.append(group)
            s["groups"] = groups
            s["duplicate_count"] = sum(len(group["entries"]) - 1 for group in groups)
            s["purged"] = s.get("purged", 0) + purged
            s["purge_status"] = "completed"
            LOGGER.info("[Duplicates] cleanup completed — removed %s", purged)
        except Exception as exc:
            s["purge_status"] = "error"
            s["error"] = str(exc)
            LOGGER.error("[Duplicates] cleanup error: %s", exc)
        finally:
